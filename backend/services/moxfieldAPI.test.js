...
test("importa correctamente un deck con primer", { timeout: TIMEOUT }, async () => {
    ...
    assert.ok(Array.isArray(data.commander) && data.commander.length > 0, "Debe tener al menos un comandante");
    assert.ok(Array.isArray(data.cards) && data.cards.length > 0, "Debe tener cartas");
    assert.ok(data.decktech_markdown && data.decktech_markdown.length > 0, "Debe tener primer (decktech_markdown)");
...
test("falla con un deck privado", { timeout: TIMEOUT }, async () => {
    const [ok, msg, data] = await importFromMoxfield("https://moxfield.com/decks/ez7FqjDaGEO9F97BKCbeYw");
...
test("importa correctamente un deck sin primer", { timeout: TIMEOUT }, async () => {
    const [ok, msg, data] = await importFromMoxfield("https://moxfield.com/decks/2HvUHT2OwEqC3HzrwWFKbg");
...
test("falla con un deck que no es Commander", { timeout: TIMEOUT }, async () => {
    const [ok, msg, data] = await importFromMoxfield("https://moxfield.com/decks/5DzIRCVKY0y-v7lFZcRFcQ/primer");

    assert.equal(ok, false, "Un deck no-Commander debería devolver ok=false");
    assert.equal(data, null, "No debe devolver datos");
    assert.ok(msg.toLowerCase().includes("commander"), `El mensaje debe mencionar Commander. Fue: "${msg}"`);
