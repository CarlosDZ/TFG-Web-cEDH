    ...
    const comentarios = await Comentario.aggregate([
      {
        $match: {
          ...
          $or: [
            { comentingOnCommanderTech: false },
            { comentingOnCommanderTech: { $exists: false } },
          ],
        },
      },
      {
...
const delete_comment = async (req, res) => {
  try {
    ...
    if (!comentarioToDelete)
      ...
      comentarioToDelete.authorId.toString() !== req.user.id &&
      !req.user.isAdmin
    )
...
const search_discusiones = async (req, res) => {
  try {
    ...
    const comentarios = await Comentario.aggregate([
      { $match: { parentId: null, comentingOnDeck: false, title: regex } },
      {
        $lookup: {
          from: "comments",
          localField: "_id",
      ...
      { $addFields: { replyCount: { $size: "$replies" } } },
      { $project: { replies: 0, likedBy: 0, __v: 0 } },
      {
        $lookup: {
          from: "users",
          localField: "authorId",
...
const obtener_discusiones_por_usuario = async (req, res) => {
  try {
    const mongoose = require("mongoose");
    const comentarios = await Comentario.aggregate([
      {
