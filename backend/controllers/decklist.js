...
const obtener_decklist = async (req, res) => {
  try {
    const decklist = await Decklist.findById(req.params.id).populate(
      "authorId",
      "username",
    );
    ...
    // Private deck: only the author can see it
    ...
    if (!myself)
      return res.status(403).json("No tienes acceso a esta decklist");
    if (decklist.authorId._id.equals(myself._id)) {
      return res.status(200).json(decklist);
    }
    return res.status(403).json("No tienes acceso a esta decklist");
  } catch (err) {
...
const get_deck_comments = async (req, res) => {
  try {
    const deck = await Decklist.findById(req.params.id).select("comments");
    if (!deck) return res.status(404).json("Decklist no encontrada");

    const mongoose = require("mongoose");
    const objectIds = deck.comments.map(
      (id) => new mongoose.Types.ObjectId(id),
    );

    const Comentario = require("../models/Comment");
    const comments = await Comentario.aggregate([
      ...
      { $unwind: "$authorId" },
      { $project: { "authorId.password": 0, "authorId.salt": 0 } },
    ]);
...
const is_liked_decklist = async (req, res) => {
  try {
    const deck = await Decklist.findById(req.params.id).select("likedBy");
    if (!deck) return res.status(404).json("Decklist no encontrada");
    const liked = deck.likedBy.some((id) => id.toString() === req.user.id);
...
const toggle_like = async (req, res) => {
  try {
    ...
    if (!decklistToLike) return res.status(404).json("Decklist no encontrada");
      ...
      decklistToLike.likedBy.some((userId) => userId.toString() === req.user.id)
    ) {
...
const reply_to = async (req, res) => {
  try {
    ...
    if (!deck) {
      ...
      typeof req.body.markdown_text !== "string"
    ) {
